...
class CallbackFormatTest extends TestCase
{
    ...
    /**
     * @dataProvider provideCallbackFormats
     * @param string $data
     ...
     * @return void
     */
    public function testFormats($data, $paymentId, $paymentStatus, $payment)
    {
        try {
            $callback = Callback::fromRaw($data, new SignatureHandler('123'));
        } catch (SdkException $e) {
            self::fail($e->getFormattedMessage());
        }

        self::assertEquals($payment, $callback->getPayment());
        self::assertEquals($paymentId, $callback->getPaymentId());
        self::assertEquals($paymentStatus, $callback->getPaymentStatus());
    }
     ...
     */
    public function testFormatsProcessException($data)
    {
        self::expectException(ProcessException::class);
        Callback::fromRaw($data, new SignatureHandler('wrong secret'));
    }
     ...
     */
    public function testFormatsArgumentException()
    {
        self::expectException(InvalidStringException::class);
        Callback::fromRaw(['test'], new SignatureHandler('wrong secret'));
    }

    /**
     * @return Generator
     */
    public function provideCallbackFormats()
    {
        yield 'Base callback' => [
            '{
                "general":{
        ...
        yield 'With converted sum callback' => [
            '{
                "general":{
